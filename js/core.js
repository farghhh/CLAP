...
/* ============================================================
   API ERROR CLASS — must be defined before API object
   ============================================================ */
...
const Validate = {
  rules: {
    ...
    // FIX: wrap combined conditions in parentheses to ensure correct precedence
    positiveInt: (v)   => (/^\d+$/.test(v) && parseInt(v) > 0) || 'Must be a positive number.',
    // FIX: compare date strings only (strip time component) so today's date is valid
    futureDate:  (v)   => {
