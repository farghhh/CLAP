...
const AppShell = {
  init(activePage, opts = {}) {
    ...
    if (window.CLAP && typeof CLAP.initSidebar === 'function') {
      CLAP.initSidebar();
    }
  ...
  _injectSidebar(activePage) {
    ...
    const user = (window.CLAP && CLAP.Auth && CLAP.Auth.getUser) ? (CLAP.Auth.getUser() || {}) : {};
  ...
  _injectLogoutHandler() {
    ...
    confirm.addEventListener('click', async () => {
      ...
      try {
        if (window.CLAP && CLAP.Auth && CLAP.Auth.getRefreshToken && CLAP.API && CLAP.API.post) {
          const refresh = CLAP.Auth.getRefreshToken();
          if (refresh) await CLAP.API.post('/auth/logout/', { refresh });
        }
      } catch {}

      if (window.CLAP && CLAP.Auth && CLAP.Auth.clearTokens) {
        CLAP.Auth.clearTokens();
      }

      if (window.CLAP && CLAP.Toast) {
